...
export const StatsView: React.FC<StatsViewProps> = ({
  ...
  const conicStyle = {
    background: trackedCustomersTotal > 0 ? `conic-gradient(${gradientParts})` : '#fce7f3',
  };
  ...
  return (
    <div className="flex-1 bg-white/90 m-3 rounded-2xl shadow-sm flex flex-col overflow-y-auto border border-gray-100 custom-scrollbar backdrop-blur-sm">
      <div className="p-8 pb-6 border-b border-gray-100 bg-gradient-to-r from-blue-50 via-pink-50 to-emerald-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white border border-gray-100 p-6 rounded-2xl shadow-sm">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="text-xs font-bold bg-red-100 text-red-600 px-2 py-1 rounded-full">סיכון</span>
            </div>
            <div className="text-3xl font-bold text-red-600 mb-1">₪{potentialLoss.toLocaleString()}</div>
            <div className="text-gray-500 text-sm">הפסד פוטנציאלי מלקוחות באיחור</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          <div className="bg-white border border-gray-100 p-5 rounded-2xl flex flex-col justify-between">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-bold text-gray-600">סך הלקוחות</span>
            </div>
            <div className="text-3xl font-bold text-gray-800">{totalCustomers}</div>
            <div className="text-xs text-gray-400 mt-1">לקוחות במערכת</div>
          </div>
          ...
          <div className="bg-white border border-gray-100 p-5 rounded-2xl flex flex-col justify-between">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-bold text-gray-600">תורים פעילים</span>
            </div>
          ...
          <div className="bg-white border border-gray-100 p-5 rounded-2xl flex flex-col justify-between">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-bold text-gray-600">תורים שבוטלו</span>
            </div>
          ...
          <div className="bg-white border border-gray-100 p-5 rounded-2xl flex flex-col justify-between">
            ...
            <div className="text-xs text-gray-400 mt-1">מתוך כלל התורים החודש</div>
          </div>
          ...
          <div className="bg-white border border-red-100 bg-red-50/10 p-5 rounded-2xl flex flex-col justify-between">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="font-bold text-red-800">אובדן מביטולים</span>
            </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800">פילוח סטטוס לקוחות</h3>
                <p className="text-xs text-gray-400">מתוך {totalCustomers} לקוחות במערכת</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white border border-red-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-red-50 bg-red-50/30 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-800">לקוחות באיחור ({lateCustomers.length})</h3>
              </div>
            </div>

            <div className="flex-1 overflow-y-auto max-h-[300px]">
              {lateCustomers.length === 0 ? (
                <div className="p-10 text-center text-gray-400">אין לקוחות באיחור!</div>
              ) : (
                <table className="w-full text-sm text-right">
                  <thead className="text-xs text-gray-400 font-medium bg-gray-50 sticky top-0">
                    <tr>
                      ...
                      <th className="px-5 py-3">איחור</th>
                    </tr>
                  </thead>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden flex flex-col">
            <div className="p-5 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-800">לקוחות מתקרבים ({soonCustomers.length})</h3>
              </div>
              <span className="text-xs text-gray-400">בשבוע הקרוב</span>
            </div>

            <div className="flex-1 overflow-y-auto max-h-[300px]">
              {soonCustomers.length === 0 ? (
                <div className="p-10 text-center text-gray-400">אין לקוחות שמתקרבים למועד היעד</div>
              ) : (
                <table className="w-full text-sm text-right">
                  <thead className="text-xs text-gray-400 font-medium bg-gray-50 sticky top-0">
                    <tr>
                      <th className="px-5 py-3">לקוח</th>
                      <th className="px-5 py-3">תאריך יעד</th>
                      <th className="px-5 py-3">נותרו</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {soonCustomers.map(c => {
